...
rewrite /weight /rows.  
induction (enum 'I_p). (* call induction*)
simpl; done. (*rewrite by simpl. left side and done. *)
simpl. (* simplify to get induction hypothesis *)
rewrite IHl. (*rewrite by induction hypothesis *)
rewrite bxorb0. (*remove the 0%:B on the left for convenience *)
apply n2b0. (* solve equality using lemma *)
Qed.
(* -------------------------------------------------------------------- *)
...
Proof.
induction r. (*call induction on r for arbitrary s*)
simpl. (*simplify the rhs*)
rewrite bxor0b. (*finish proof*)
done.
simpl. (*simplify to enable induction hypothesis application*)
rewrite IHr. (*rewrite to get two identical expressions *)
apply bxorA. (*apply associativity to finish*)
Qed.
...
Proof.
move=>m.
case (RP m) => [p] [q] [h1 h2 h3]. (*case to get hypotheses from RP*)
rewrite bxorC. (*simplifying everything*)
rewrite /weight.
rewrite h3 h2.
rewrite weight_rD.
rewrite weight_rS.
-rewrite weight_rD.
-rewrite weight_rS.
rewrite bxorC. (*rewriting everything to use bxor lemmas later*)
rewrite bxorA.
-rewrite -!bxorA.
 congr (_ .+ _).
rewrite bxorC.
...
move =>m.
(*first we get s2 in terms of s1*)
assert (lemma: weight s2 = weight s1 .+ n2b (s1 i) .+ n2b (s2 i)).
apply (turn_weight).
apply m.
(*rewrite so we can begin removing s1*)
rewrite lemma.
move =>s10.
rewrite s10.
rewrite s10 in lemma.
rewrite bxor0b in lemma.
rewrite bxor0b.
move: s10.
apply contra_eq_not.
(*since weight s2 is the nimsum of s1 and s2, we want to prove that the nimsum is non-zero*)
move =>h1.
rewrite h1 in lemma.
rewrite bxorC in h1.
(*we define this lemma to move s1 to the rhs so we can apply injectivity later*)
assert(lemma3: n2b (s2 i) .+ n2b (s1 i) = 0%:B -> n2b (s2 i) .+ n2b (s1 i) .+ n2b (s1 i) = 0%:B .+ n2b(s1 i)).
move =>help.
rewrite help.
done.
apply lemma3 in h1.
...
rewrite -bxorA in h1.
rewrite bxorbb in h1.
rewrite bxorb0 in h1.
apply n2b_inj in h1.
(*destruct m to gain contradiction to prove lemma2*)
destruct m.
rewrite h1 in H.
assert( falseassump := ltnn (s1 i)).
rewrite H in falseassump.
discriminate.
Qed.
(* -------------------------------------------------------------------- *)
...
Proof.
move => weight_s.
(*we apply hibit to show that there is "1" at position d*)
apply hibit_neq0W in weight_s.
remember ((size (weight s)).-1) as d.
(*we define a lemma that says that there exists k, such that x_k.d is also "1"*)
assert (exists_k: exists (i: 'I_p), (n2b(s i)).[d]=true).
apply/existsP/existsPn => /=.
move => x_kd.
(*we unfold weight and rows to do induction on 'I_p*)
unfold weight in weight_s.
unfold rows in weight_s.
(*we do induction on our weight_s*)
move: weight_s.
induction (enum 'I_p).
(*reduce to weight of empty bit vector*)
simpl.
rewrite b0E;done.
(*simplify by weight_rS pattern*)
simpl.
rewrite bxorE.
assert (spec_x_kd:~~(n2b (s a)).[d]).
apply x_kd.
apply negbTE in spec_x_kd.
(*equal the two parts for false to get result*)
assert (impl_false: ((weight_r [seq s i | i <- l]).[d] -> False) -> ((weight_r [seq s i | i <- l]).[d] = false)).
move => h1.
destruct (weight_r [seq s i | i <- l]).[d];try done.
exfalso.
apply h1.
done.
apply impl_false in IHl.
(*rewrite newly gained false statements in the goal to finish*)
rewrite spec_x_kd.
rewrite IHl.
done.
(*second part*)
(*destruct exists_k to get two separate hypotheses for later*)
destruct exists_k as [i ek1].
exists i.
(*we construct the y_k*)
remember (n2b (s i)) as x_k.
remember ((weight s).+x_k) as y_k.
(*we construct s'*)
remember(fun k: 'I_p => if k!=i then s k else b2n y_k ) as s'.
exists s'.
(*we prove y_k<x_k*)
assert(yk_xk : (b2n(y_k)) < (b2n(x_k))).
(*put back their original values*)
rewrite Heqx_k.
rewrite Heqy_k.
(*solve through lt_n2b*)
apply lt_n2b.
exists d.
move => i0 di0.
rewrite bxorE.
rewrite Heqd in di0.
assert(ws_i0: (size (weight s)) <=i0).
(*call induction on size(weight s)*)
induction (size(weight s)).
trivial.
apply di0.
(*we need that i0 in state vector is 0*)
rewrite (bit_oversize ws_i0).
rewrite Heqx_k.
apply addFb.
rewrite bxorE.
rewrite weight_s.
...
rewrite ek1.
trivial.
(*first we prove that we can move from s to s'*)
assert (canmove: R i s s').
(*we deconstruct the move statement to obtain conditions*)
constructor.
(*first part*)
(*we rewrite s' in it's original form to try and prove the inequality*)
rewrite Heqs'.
(*now we try to make LHS just b2n y_k*)
assert(i_i: (i!=i)=false).
by apply /eqP => //.
rewrite i_i.
rewrite Heqx_k in yk_xk.
rewrite n2bK in yk_xk.
apply yk_xk.
move => i1 i1i.
(*second part*)
(*again expand s'*)
...
(*now we do the final part*)
split.
Focus 2.
assumption.
(*expand the canmove*)
apply turn_weight in canmove.
rewrite canmove.
rewrite Heqs'.
simpl.
assert(i_i: (i!=i)=false).
